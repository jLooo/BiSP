...
import argparse
from model.MGM_shanghaitech import *
...
def Eval(model=None):
    ...
    parser.add_argument('--dataset_type', type=str, default='shanghaitech', help='type of dataset: shanghaitech')
    parser.add_argument('--dataset_path', type=str, default='D:/Datasets', help='directory of data')
    parser.add_argument('--model_dir', type=str, default='./modelzoo/best_model_shtech.pth', help='directory of model')
    ...
    if model is None:  # if not training, we give a exist model and params path
        model = MGM_shanghai(args.t_length // 2, args.c)
        try:
    ...
    # np.save('./data/psnr/sht/{}_{}.npy'.format(args.dataset_type, str(accuracy)), psnr_multi_list)
